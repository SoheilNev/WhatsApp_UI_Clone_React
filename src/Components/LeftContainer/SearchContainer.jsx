...
function SearchContainer(props) {
  ...
  return (
    <div className="relative flex w-full h-fit items-center bg-main">
      <div className="relative flex content-center w-full box-border px-3 py-2 bg-main ">
				<div className="relative flex w-full h-10 items-center rounded-xl box-border border-green bg-headers">
					...
					<input onFocus={() => setIconChange("arrow")} onBlur={() => setIconChange("search")} className="text-base w-11/12 h-7 pl-16 pr-8 box-border focus:outline-none text-[#918AB0] bg-headers border-left-search-cont-input"  type="text" placeholder="Search or start a new chat" />
				</div>
			</div>
      <button onClick={(e) => {props.FilterCallBack()}} className="mr-2" type="button" placeholder="Unread chats filter">
        <div className={"p-1 rounded-full "+( props.filter ? "text-main bg-green": "text-left-header-nav-icons")}>
